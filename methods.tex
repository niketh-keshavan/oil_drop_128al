...
Two features of the data are notable. First, the falling velocities are considerably more consistent than the rising velocities: fall velocity standard deviations are 3--11\% of the mean, while rise velocity standard deviations range from 5\% up to 41\% of the mean. This asymmetry is expected because the falling phase depends only on the droplet mass and air viscosity (both constant), whereas the rising phase depends additionally on the electric force $qE$, making it sensitive to any charge fluctuations during the recording. The large scatter in some rise velocities (e.g., Trial~2, Drop~1, where individual rise velocities range from 0.139 to 0.363~mm/s) suggests that stray ionization from ambient radiation may have altered the droplet charge during the measurement window.
...
The corrected droplet radii range from 0.77 to 1.31~$\mu$m, well within the regime where the Cunningham slip correction is significant. The measured charges span from $8.58 \times 10^{-19}$~C to $19.52 \times 10^{-19}$~C, corresponding to integer multiples of the elementary charge ranging from $n = 5$ to $n = 12$. Crucially, no charges were observed at non-integer multiples of $e$: the $q/e$ ratios cluster tightly around integers (within measurement uncertainty), providing strong evidence for charge quantization.

Figure~\ref{fig:quantization} shows all six measured charges expressed in units of $e$, with horizontal reference lines at integer values. Each measurement falls within its uncertainty of a nearby integer, with no charges observed between quantization levels.

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.8\textwidth]{charge_quantization.png}
    \caption{Charge quantization pattern showing all six measured charges in units of the elementary charge. Horizontal dashed lines mark integer multiples of $e$. Pre-ionization measurements (circles) and post-ionization measurements (squares) fall at distinct quantization levels ($n = 5, 8, 9, 12$), with no intermediate values.}
    \label{fig:quantization}
\end{figure}

\subsection{Elementary Charge Determination}

The elementary charge is extracted by dividing each measured charge by its assigned integer quantum number $n$ and computing a weighted average. For each droplet, the per-droplet estimate is $e_i = q_i / n_i$ with uncertainty $\delta e_i = \delta q_i / n_i$. The individual estimates are:

\begin{table}[h!]
    \centering
    \caption{Per-Droplet Elementary Charge Estimates}
    \vspace{0.1in}
    \begin{tabular}{|c|c|c|c|}
    \hline
    \textbf{Droplet} & \textbf{$n$} & \textbf{$q/n$ ($10^{-19}$ C)} & \textbf{$\delta(q/n)$ ($10^{-19}$ C)} \\
    \hline
    T1D1 & 12 & 1.604 & 0.202 \\
    T1D2 &  8 & 1.668 & 0.217 \\
    T2D1 &  5 & 1.716 & 0.234 \\
...
The weighted average (with weights $w_i = 1/\delta e_i^2$) yields:
\begin{equation}
e_{\text{exp}} = (1.603 \pm 0.051) \times 10^{-19} \text{ C}
\end{equation}

The CODATA 2018 value is $e_{\text{CODATA}} = 1.602176634 \times 10^{-19}$~C. Our result agrees to within 0.05\%, well within the 3.2\% measurement uncertainty. Figure~\ref{fig:per_drop_e} shows the per-droplet estimates, illustrating that all six independent measurements are consistent with the accepted value.

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.75\textwidth]{per_drop_e_estimate.png}
    \caption{Per-droplet elementary charge estimates ($q/n$) for all six droplets. The red line marks the CODATA 2018 value; the blue band shows the $\pm 1\sigma$ range of the weighted average. All measurements are consistent with the accepted value.}
...
\begin{figure}[h!]
    ...
    \caption{Experimental result $(1.603 \pm 0.051) \times 10^{-19}$~C agrees with the CODATA 2018 value to within 0.05\%, confirming the accuracy of the measurement despite relatively large per-droplet uncertainties.}
    \label{fig:result_comparison}
\end{figure}

\subsection{Summary of Results}

The elementary charge was determined from six oil droplet measurements across three independent trials to be:
\begin{equation}
e_{\text{measured}} = (1.603 \pm 0.051) \times 10^{-19} \text{ C}
\end{equation}

This result agrees with the CODATA 2018 value of $1.602176634 \times 10^{-19}$~C to within 0.05\%, well within the 3.2\% measurement uncertainty. Measured charges ranged from $5e$ to $12e$, and in every case the ratio $q/e$ fell within measurement uncertainty of an integer, providing strong evidence for the quantization of electric charge. The ionization procedure changed droplet charges by 3--4 elementary charges per trial, with both charge gain and loss observed, consistent with the stochastic nature of the ionization process. The measurement is fundamentally limited by velocity scatter ($> 99.9\%$ of the uncertainty budget), and future improvements should focus on higher frame-rate video capture or automated position tracking to reduce this dominant error source.
